...
class WorkloadUsageReportProvider extends AbstractReportProvider {
	...
	void process(ReportResult reportResult) {
		...
		try {
			...
			results = new Sql(dbConnection).rows("SELECT compute_server.id,compute_server.name,compute_server.provision,compute_server.status,compute_server.server_type,compute_server.zone_id,compute_server.managed,compute_server.server_type,compute_server.status,compute_server.discovered,compute_server.account_id,compute_server.power_state,compute_server_type.name as server_type_name, compute_zone.name as cloud_name, account.name as account_name FROM morpheus.compute_server LEFT JOIN account ON compute_server.account_id = account.id LEFT JOIN compute_zone on compute_server.zone_id = compute_zone.id INNER JOIN compute_server_type ON compute_server.compute_server_type_id=compute_server_type.id WHERE compute_server.compute_server_type_id in (select id from compute_server_type where managed = 0 and container_hypervisor = 0 and vm_hypervisor = 0 ) order by id asc;")
			instances = new Sql(dbConnection).rows("SELECT instance.id, instance.name, compute_zone.name as cloud_name, account.name as account_name FROM morpheus.instance LEFT JOIN compute_zone on instance.provision_zone_id = compute_zone.id LEFT JOIN account ON instance.account_id = account.id order by id asc;")
